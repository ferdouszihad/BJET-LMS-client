import { Link, NavLink } from "react-router-dom";
import { useContext } from "react";
import { AuthContext } from "../providers/AuthProvider";
import { ThemeContext } from "../providers/ThemeProvider";
import { CgProfile } from "react-icons/cg";
import Logo from "./Logo";
const Navbar = () => {
  const { theme, toggleTheme } = useContext(ThemeContext);
  const { user, logOut } = useContext(AuthContext);
  const routes = [];
  return (
    <nav className="border-b-2 border-base-200">
      {user && (
        <div className="bg-neutral text-neutral-content text-center p-1 ">
          Welcome {user?.displayName?.split(" ")[0]}.🐦‍🔥 Best of Luck gor your
          learning
        </div>
      )}

      <div className="navbar bg-base-100 md:w-11/12 mx-auto">
        <div className="navbar-start ">
          <Link className="logo flex flex-col  items-center gap-2" to={"/"}>
            <Logo></Logo>
            <div className="badge badge-primary badge-sm">Learn and Shine</div>
          </Link>
        </div>
        <div className="navbar-center hidden lg:flex">
          <ul className="menu-horizontal px-1 gap-5 ">
            {routes.map((navlinks) => (
              <li key={navlinks.path}>
                <NavLink
                  className={"flex items-center gap-2"}
                  to={navlinks.path}
                >
                  {navlinks.icon}
                  {navlinks.name}
                </NavLink>
              </li>
            ))}
          </ul>
        </div>
        <div className="navbar-end space-x-2">
          {user && user?.email ? (
            <div className="flex gap-2 ">
              <div className="dropdown dropdown-hover">
                <div
                  role="button"
                  tabIndex={0}
                  className="w-12 h-12 p-[2px] border-2 border-green-400 rounded-full relative"
                >
                  <div className="absolute inset-1 bg-primary text-primary-content rounded-full flex items-center justify-center ">
                    {user?.email && user?.email[0]}
                  </div>
                  <img
                    className="rounded-full w-full h-full relative z-10"
                    src={user?.photoURL}
                    alt=""
                  />
                </div>
                <ul
                  tabIndex={0}
                  className="dropdown-content  bg-base-100 rounded-box z-[1] w-40 p-2 shadow"
                >
                  <li>
                    <Link className="flex gap-2 items-center" to="/dashboard">
                      <CgProfile></CgProfile> User-Profile
                    </Link>
                  </li>
                </ul>
              </div>
              <div className="space-y-1">
                <h2 className="text-sm">{user.displayName}</h2>
                <button onClick={logOut} className="btn btn-xs btn-primary ">
                  Log-Out
                </button>
              </div>
            </div>
          ) : (
            <>
              <NavLink
                to={"/auth/login"}
                className="btn btn-outline  btn-primary btn-sm"
              >
                Login
              </NavLink>
              <NavLink
                to={"/auth/register"}
                className="btn btn-outline btn-primary btn-sm"
              >
                Register
              </NavLink>
            </>
          )}

          <label
            onClick={() =>
              toggleTheme(theme.mode === "light" ? "light" : "dark")
            }
            className=" cursor-pointer"
          >
            {theme.mode == "light" && (
              <svg
                className="swap-on h-10 w-10 fill-current"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
              >
                <path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
              </svg>
            )}

            {theme.mode == "dark" && (
              <svg
                className="swap-off h-10 w-10 fill-current"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
              >
                <path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
              </svg>
            )}

            {/* Moon icon */}
          </label>
        </div>
      </div>
    </nav>
  );
};

export default Navbar;
